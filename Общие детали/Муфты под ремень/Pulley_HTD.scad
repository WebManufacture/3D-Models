...
motor_shaft = 13.7;	// NEMA17 motor shaft exact diameter = 5
...
pulley_t_ht = 40;	// length of toothed part of pulley, standard = 12
pulley_b_ht = 8;		// pulley base height, standard = 8. Set to same as idler_ht if you want an idler but no pulley.
pulley_b_dia = 8.1;	// pulley base diameter, standard = 20
    ...
    difference(){
		union()
		{
			...
			if ( pulley_b_ht < 2 ) { echo ("CAN'T DRAW PULLEY BASE, HEIGHT LESS THAN 2!!!"); } else {
                difference(){
                    cylinder(d=pulley_OD,h=pulley_b_ht, $fn=teeth*4);
                    translate([0,0,0]) 
                    cylinder(d=pulley_b_dia,h=pulley_b_ht, $fn=teeth*4);
                }
			}
