...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
    ...
    difference(){
		...
		if ( no_of_nuts <= 0 ) { echo ("CAPTIVE NUTS = 0"); } else {
            if ( pulley_b_ht < m3_nut_flats ) { echo ("CAN'T DRAW CAPTIVE NUTS, HEIGHT LESS THAN NUT DIAMETER!!!"); } else {
                if (pulley_OD/2 < m3_nut_depth ) { echo ("CAN'T DRAW CAPTIVE NUTS, DIAMETER TOO SMALL FOR NUT DEPTH!!!"); } else {
                    ...
                    union()
                    {
                        ...
                        //grub screw hole
                        rotate([0,0,22.5])cylinder(r=m3_dia/2,h=pulley_OD/2 + 2,$fn=8);
                    }
                }
             }
     }
